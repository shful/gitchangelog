...
def paragraph_wrap(text, regexp="\n\n"):
    ...
    return "\n".join("\n".join(textwrap.wrap(paragraph.strip(), textwrap_width))
...
def rest_py(data, opts={}):
    ...
    def render_commit(commit, opts=opts):
        ...
        entry = indent('\n'.join(textwrap.wrap(subject, textwrap_width)),
                       first="- ").strip() + "\n\n"
        if commit["body"]:
            entry += "\n" + indent(commit["body"])
            entry += "\n"
...
def versions_data_iter(repository, revlist=None,
    ...
    for idx, tag in enumerate(tags):
        ...
        # bodies is a map {body:commit}
        # This is to detect duplicate commits (e.g. cherry-picks)
        bodies = {}
        ...
        for commit in commits:
            ...
            # Consider only non-ignored commits.
            bodydup_txt = ""
            if len(commit.body.strip()) > 0:
                if commit.body in bodies:
                    first = bodies[commit.body]
                    d = datetime.datetime.utcfromtimestamp(
                        float(first.committer_date_timestamp))
                    bodydup_txt = "! Duplicate text. First committed by " + first.committer_name + " as " + first.sha1_short + " on " + d.strftime('%Y-%m-%d') + '\n'
                else:
                    bodies[commit.body] = commit
            ...
            sections[matched_section].append({
                ...
                "subject": subject_process(commit.subject.rstrip()),
                "body": body_process(commit.body) + revert_txt + bodydup_txt,
                "commit": commit,
            })
